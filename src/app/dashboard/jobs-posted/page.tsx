...
export default function JobsPostedPage() {
  ...
  // REMOVED: const [userId, setUserId] ... (Not needed for fetching anymore)
  ...
  // --- HELPER FUNCTIONS (getSignedResumeUrl, handleDownload, handleView, viewApplicants) ---
  // (Keep these exactly as they were in your code)
  async function getSignedResumeUrl(jobId: string, appId: string) {
    if (!token) { toast.error("You must be logged in"); return null }
    try {
      const res = await fetch(`/api/jobs/${jobId}/applications/${appId}/resume`, {
        method: "GET", headers: { "Authorization": `Bearer ${token}` },
      })
      if (!res.ok) { const err = await res.json(); toast.error(err.error || "Failed"); return null }
      const data = await res.json(); return data.url 
    } catch (error) { console.error(error); return null }
  }
  async function handleDownload(appId: string) { if (!selectedJobId) return; const url = await getSignedResumeUrl(selectedJobId, appId); if (url) window.open(url, '_blank') }
  async function handleView(appId: string) { if (!selectedJobId) return; const url = await getSignedResumeUrl(selectedJobId, appId); if (url) { setPreviewUrl(url); setResumePreviewOpen(true) } }

  async function viewApplicants(jobId: string) {
    if (!token) return
    setLoadingApplicants(true); setSelectedJobApplicants([]); setSelectedJobId(jobId)
    try {
      const res = await fetch(`/api/jobs/${jobId}/applications`, { headers: { Authorization: `Bearer ${token}` } })
      const data = await res.json()
      if (res.ok) { setSelectedJobApplicants(data.applications || []); setApplicantDialogOpen(true) } 
      else { toast.error(data.error || "Failed to load applicants") }
    } catch (error) { console.error(error); toast.error("Network error") } 
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        setIsEmployee(true)
        // FIX: No need to wait for ID or set state. Just load "me".
        await loadJobs() 
  ...
  // FIX: Simplified function. No arguments needed.
  async function loadJobs() {
    try {
      // Use 'me' magic string. The backend will use the token to find the ID.
      const res = await fetch(`/api/jobs?postedBy=me`, {
        headers: { Authorization: `Bearer ${token}` },
      })
  ...
  async function submitJob() {
    ...
    try {
      ...
      setForm({
        title: "", company: "", location: "", jobType: "", remote: false,
        description: "", requirements: "", salaryMin: "", salaryMax: "",
      })

      // FIX: Just reload. No arguments needed.
      loadJobs()
  ...
  return (
    <main className="flex-1 overflow-y-auto p-4 md:p-6">
      <div className="max-w-4xl mx-auto space-y-6">
        {/* ... JOB POSTING HEADER AND FORM (Same as before) ... */}
        ...
        {jobs.length === 0 ? (
          <Card className="border-dashed">
            <CardHeader>
              <CardTitle>No jobs posted by you yet</CardTitle>
          ...
          <div className="space-y-4">
            ...
            {/* APPLICANTS LIST DIALOG (Same as provided) */}
            ...
            {/* RESUME PREVIEW DIALOG (Same as provided) */}
            <Dialog open={resumePreviewOpen} onOpenChange={setResumePreviewOpen}>
              <DialogContent className="max-w-4xl h-[85vh] flex flex-col">
                ...
                <div className="flex-1 bg-muted/10 rounded-md overflow-hidden border">
                  {previewUrl ? (
                    <iframe src={previewUrl} className="w-full h-full border-none" title="Resume" />
                  ) : (
